...
export default function WidgetItem({ widget, status = "NOT_INSTALLED" }) {
    return (
        <>
            <s-app-window id={`edit-window${widget?.id}`} src={widget?.editUrl}></s-app-window>
            <CustomSection background="#fff" border="none" boxShadow="none" padding="none" overflow="hidden">
                ...
                <s-grid gap="small" padding="small base">
                    ...
                    <s-stack direction="inline" justifyContent="space-between" alignItems="center">
                        ...
                        <s-button icon="paint-brush-round"  command="--show" commandFor={`edit-window${widget?.id}`}>Customize</s-button>
                    </s-stack>
                </s-grid>
            </CustomSection>
        </>
