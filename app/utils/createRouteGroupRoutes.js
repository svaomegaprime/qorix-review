...
export function createRouteGroupRoutes(routeFileUrl) {
  const routeGroupDirectory = path.dirname(fileURLToPath(routeFileUrl));
  const routeGroupName = path.basename(routeGroupDirectory);
  const routesDirectory = `routes/${routeGroupName}/routes`;
  const routesFolderPath = path.join(routeGroupDirectory, "routes");
  ...
  const addRoute = ({
    ...
    routes.push(
      children.length > 0
        ? route(urlPath, filePath, children)
  ...
  return collectRoutes(routesFolderPath);
}
