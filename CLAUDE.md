...
> This repo also has an `AGENTS.md` with the same guidance — keep both in sync when updating.
...
- **Single npm workspace**: `extensions/*` (one theme extension at `extensions/qorix-review/` — the storefront-embedded widget code, separate from the admin app).
- App is a Shopify **product reviews / widget platform**: models include `QuickReviewWidget`, `QuoteLoopWidget`, `TrustBarWidget` (all on `Store`), plus `Review`, `Order`, `StoreSettings`, `Subscription`.
...
- **BullMQ job queue** backed by **Redis** (localhost:6379). Worker runs as a separate process via `npm run worker`. Docker Compose only provisions Redis — the app process itself is not containerized in that compose file. Note: `docker-start` chains `setup && start && worker`, but `start` runs the server indefinitely, so `worker` never actually launches in that chain — run the worker as its own process/container in practice.
...
Storefront-facing/API routes are prefixed `api.*` (`api.helpful`, `api.review`, `api.unsubscribe`, `api.upload`), and webhook handlers live at `webhooks.*` (`webhooks.app.scopes_update`, `webhooks.app.uninstalled`, `webhooks.orders.cancelled`, `webhooks.orders.create`, `webhooks.orders.updated`).
